                    ...
                    <div class="card">
                        <div class="header">
                            <div class="x_title">
                                ...
                                <a href="{{route('BookController.create')}}">
                                ...
                                <tbody>
                                    @foreach($books as $book)
                                        <tr>
                                        <td>{{$book->id}}</td>
                                        <td><img src="{{ asset('storage/images/book_image/'.$book->image_link) }}"  style="  width:50px;height:50px;border-radius: 50% ;" alt="Avatar"/></td>
                                        <td>{{$book->title}}</td>
                                        <td>{{$book->language}}</td>
                                        ...
                                        <td>{{$book->author->name}}</td>
                                            <td>
                                                <div class="btn-group">
                                                <form action="{{route('BookController.destroy',$book->id)}}" method="post">
                                                    @csrf
                                                    @method('delete')
